public class Recursion {
    ...
    public static double computePyramidWeights(double[][] weights, int row, int column) {
        ...
        if (column == 0)
            return weights[row][column]
                    + ((computePyramidWeights(weights, row - 1, column) / 2));
        return weights[row][column]
    ...
    public static int howManyOrganisms(char[][] image) {
        ...
        for (int i = 0; i < image.length; i++) {
            for (int j = 0; j < image[i].length; j++) {
                if (image[i][j] == '*') {
                    findOrganisms(image, letter, new int[]{i, j});
                    organisms++;
                    letter++;
                }
            }
    ...
    public static void findOrganisms(char[][] image, char letter, int[] currCell) {
        int i = currCell[0];
        int j = currCell[1];
        // base case
        image[i][j] = letter;
        ...
        // recursive step
        if (i + 1 < image.length && image[i + 1][j] == '*') {
            findOrganisms(image, letter, new int[] {i + 1, j});
        }
        if (i - 1 >= 0 && image[i - 1][j] == '*') {
            findOrganisms(image, letter, new int[] {i - 1, j});
        }
        if (j + 1 < image[i].length && image[i][j + 1] == '*') {
            findOrganisms(image, letter, new int[] {i, j + 1});
        }
